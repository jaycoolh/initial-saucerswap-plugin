...
import { yourCustomPlugin } from "./plugins/your-custom-plugin";
...
export function createHederaClient(bootstrap: AgentBootstrap): Client {
  const client =
    bootstrap.network === "mainnet" ? Client.forMainnet() : Client.forTestnet();
  if (bootstrap.mode === "autonomous") {
    ...
    if (!HEDERA_OPERATOR_ID || !HEDERA_OPERATOR_KEY) {
      throw new Error(
        "HEDERA_OPERATOR_ID and HEDERA_OPERATOR_KEY are required in autonomous mode"
      );
    }
    client.setOperator(
...
export function createToolkitConfiguration(
